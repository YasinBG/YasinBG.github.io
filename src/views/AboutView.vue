<template>
  <div>
    <input
      class="w-60 h-12 border-2 border-green-500 rounded mx-auto font-semibold"
      :value="newTask"
      @input="updateNewTask"
      @keyup.enter="addTask"
    ...
    <ul class="flex flex-col my-6" v-for="(task, index) in tasks" :key="index">
      <li class="flex text-start my-2 font-semibold">
    ...
    <button
      @click="recoverTask"
      class="p-2 font-semibold border-2 border-green-600 text-center uppercase rounded bg-green-400 text-white hover:bg-green-600 w-24 h-10"
    >
      geri al
    </button>
  </div>
